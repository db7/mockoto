#ifndef MOCKOTO_CHIBI_GEN_HPP
#define MOCKOTO_CHIBI_GEN_HPP
...
private:
  ...
  std::string toTypeExpr(QualType type) const {
    ...
    if (type->isPointerType()) {
      ...
      if ((p->isStructureType() || p->isUnionType()) &&
          (!isCompleteRecordOrUnion(p) || !isProcessableRecordOrUnion(p)))
        return "(pointer void)";
      if (p->isStructureType() || p->isUnionType())
        return toTypeExpr(p);
    ...
    if (const TypedefType *tt = dyn_cast<TypedefType>(type.getTypePtr())) {
      QualType ut = tt->getDecl()->getUnderlyingType();
      ut.removeLocalFastQualifiers();
      if (isIntegerLike(ut) || ut->isEnumeralType() || ut->isStructureType() ||
          ut->isUnionType())
  ...
  void emitRecord(const RecordDecl *decl) {
    ...
    for (auto it = decl->field_begin(); it != decl->field_end(); it++, idx++) {
      ...
      bool noAccessors = ftype->isArrayType() || ftype->isPointerType() ||
